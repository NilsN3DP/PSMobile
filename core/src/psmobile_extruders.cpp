...
PSM_API psm_result psm_preset_save_as(psm_session *s, psm_preset_type type, const char *name)
{
    ...
    if (c == nullptr)
        return s == nullptr ? PSM_ERR_INVALID_ARG : PSM_ERR_NOT_FOUND;
    try {
        /* PrusaSlicers save_current_preset() kehrt bei einem System-,
         * Standard- oder Projektprofil stillschweigend zurueck - der
         * Desktop laesst solche Namen gar nicht erst zu. Mobil sagte
         * "Profil ueberschreiben" bis zum 16.09.2026 Erfolg zu, tat nichts
         * und der Wechsel warf die Aenderungen weg (Emulator). */
        if (const Preset *bestehend = c->find_preset(name, false);
            bestehend != nullptr &&
            (bestehend->is_system || bestehend->is_default || bestehend->is_external)) {
            s->set_error("Systemprofil laesst sich nicht ueberschreiben");
            return PSM_ERR_INVALID_ARG;
        }
