                                   ...
                                   char *out_reason, size_t reason_cap)
{
    ...
    try {
        ...
        if (r != reason_map().end()) {
            const char *grund = r->second;
            /* thin_walls sperrt auch der Arachne-Generator (siehe
             * psmobile_toggles_generated.cpp) - mit Perimetern > 0 ist er
             * der Grund, nicht "perimeters" (16.09.2026). */
            if (std::string(key) == "thin_walls" &&
                s->config.opt_int("perimeters") > 0)
                grund = "perimeter_generator";
            copy_str(out_reason, reason_cap, grund);
        }
        return 0;
