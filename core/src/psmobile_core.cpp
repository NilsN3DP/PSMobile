    ...
    const Slic3r::GCodeProcessorResult &result)
{
    ...
    /*
     * PrusaSlicer 2.9 fuegt je Block bis zu zwei "actual speed"-Punkte
     * ein (Ende der Beschleunigung, Beginn des Bremsens): Klone des
     * Originals mit demselben delta_extruder, aber ohne Zeit
     * (GCodeProcessor::calculate_time). Wer delta_extruder ueber alle
     * Extrude-Punkte summiert, zaehlt jeden Block bis zu dreimal - der
     * Wuerfel stand mit 2,82 m / 8,4 g statt 1,21 m / 3,6 g in der
     * Vorschau (S23 FE, 16.09.2026). Bogen-Segmente (G2/G3) sind ebenfalls
     * internal_only, tragen aber ihre eigene Zeit und ihren eigenen
     * Anteil - die zaehlen.
     */
    const auto ist_geschwindigkeitsklon =
        [](const Slic3r::GCodeProcessorResult::MoveVertex &move) {
            return move.internal_only &&
                   move.time[static_cast<size_t>(
                       Slic3r::PrintEstimatedStatistics::ETimeMode::Normal)] == 0.f &&
                   move.time[static_cast<size_t>(
                       Slic3r::PrintEstimatedStatistics::ETimeMode::Stealth)] == 0.f;
        };

    for (const auto &move : result.moves) {
        if (move.type != Slic3r::EMoveType::Extrude)
            continue;
        if (ist_geschwindigkeitsklon(move))
            continue;
...
PSM_API int psm_supports_step(void)
{
#if PSM_HAS_STEP
    return 1;
#else
...
void psm_session::activate_dirs() const
{
    if (Slic3r::data_dir() != datadir)
        Slic3r::set_data_dir(datadir);
    if (Slic3r::resources_dir() != resdir) {
        Slic3r::set_resources_dir(resdir);
        Slic3r::set_var_dir((boost::filesystem::path(resdir) / "icons").string());
        Slic3r::set_local_dir((boost::filesystem::path(resdir) / "localization").string());
    }
}
                                      ...
                                      uint32_t *out_after)
{
    PSM_GUARD_BEGIN(s)
        ...
        uint64_t after = 0;
        /* Unter diese Dreieckszahl geht es nicht: ein Wuerfel (12 Dreiecke)
         * wurde sonst auf 6 zusammengeklappt - ein Keil, kein Wuerfel mehr
         * (S23 FE, 16.09.2026). PrusaSlicers eigene Untergrenze liegt in
         * derselben Groessenordnung. */
        constexpr uint32_t kMinDreiecke = 64;
        for (Slic3r::ModelVolume *volume : object->volumes) {
            if (! volume->is_model_part() || volume->mesh().facets_count() <= kMinDreiecke) {
                if (volume->is_model_part())
            ...
            const float volumen_vorher = Slic3r::its_volume(mesh);
            const uint32_t target = std::max<uint32_t>(
                kMinDreiecke, static_cast<uint32_t>(
                    std::llround(mesh.indices.size() * ratio)));
            Slic3r::its_quadric_edge_collapse(mesh, target);
            /* Und wenn das Ergebnis die Form verloren hat (Volumen um mehr
             * als ein Fuenftel veraendert), bleibt das Original stehen. */
            const float volumen_nachher = Slic3r::its_volume(mesh);
            if (! std::isfinite(volumen_nachher) || volumen_vorher <= 0.f ||
                std::abs(volumen_nachher - volumen_vorher) > 0.2f * volumen_vorher) {
                after += volume->mesh().facets_count();
                continue;
            }
...
/*
 * Ob ein Zahlenwert in den Bereich seiner Definition faellt.
 *
 * set_deserialize prueft nur die Form ("ist das eine Zahl"), nicht den
 * Sinn: eine Schichthoehe von 0 ging durch und fiel erst beim Schneiden
 * mit einer Kernmeldung um (RandfaelleUITest, 12.09.2026). Der Desktop
 * hat dafuer die Feldpruefung im GUI; hier sitzt sie im Kern, damit
 * beide Oberflaechen dieselbe Antwort bekommen.
 *
 * Geprueft werden min/max aus der ConfigOptionDef, und fuer ein paar
 * Masse, bei denen null physikalisch Unsinn ist, zusaetzlich "> 0" -
 * PrusaSlicer traegt dort min = 0 ein, meint aber "positiv".
 * Prozentwerte und Listen werden je Eintrag geprueft; Text, Bool und
 * Aufzaehlungen gar nicht.
 */
bool wert_im_bereich(const Slic3r::ConfigOptionDef *def,
                     ...
                     std::string &fehler)
{
    if (def == nullptr)
        return true;
    switch (def->type) {
    case Slic3r::coFloat: case Slic3r::coFloats:
    case Slic3r::coInt: case Slic3r::coInts:
    case Slic3r::coFloatOrPercent: case Slic3r::coFloatsOrPercents:
    case Slic3r::coPercent: case Slic3r::coPercents:
        break;
    default:
        return true;
    }
    static const char *const streng_positiv[] = {
        "layer_height", "nozzle_diameter", "filament_diameter", nullptr,
    };
    bool positiv = false;
    for (const char *const *k = streng_positiv; *k != nullptr; ++k)
        if (key == *k) { positiv = true; break; }

    std::string rest = value;
    while (! rest.empty()) {
        const size_t komma = rest.find(',');
        std::string teil = rest.substr(0, komma);
        rest = komma == std::string::npos ? std::string() : rest.substr(komma + 1);
        // Leerzeichen und ein Prozentzeichen am Ende weg.
        while (! teil.empty() && (teil.back() == ' ' || teil.back() == '%')) teil.pop_back();
        while (! teil.empty() && teil.front() == ' ') teil.erase(teil.begin());
        if (teil.empty() || teil == "nil")
            continue;
        char *ende = nullptr;
        const double zahl = std::strtod(teil.c_str(), &ende);
        if (ende == teil.c_str())
            continue;   // keine Zahl - das meldet set_deserialize
        if (positiv && zahl <= 0.0) {
            fehler = key + " muss groesser als 0 sein";
            return false;
        }
        if (def->min > -std::numeric_limits<float>::max() && zahl < def->min) {
            fehler = key + " darf nicht kleiner als " + std::to_string(def->min) + " sein";
            return false;
        }
        if (def->max < std::numeric_limits<float>::max() && zahl > def->max) {
            fehler = key + " darf nicht groesser als " + std::to_string(def->max) + " sein";
            return false;
        }
    }
    return true;
...
PSM_API psm_result psm_config_set(psm_session *s, const char *key, const char *value)
{
    PSM_GUARD_BEGIN(s)
        ...
        {
            std::string fehler;
            const Slic3r::ConfigDef *defs = s->config.def();
            if (! wert_im_bereich(defs != nullptr ? defs->get(key) : nullptr, key, value, fehler)) {
                s->set_error(fehler);
                return PSM_ERR_INVALID_ARG;
            }
        }
