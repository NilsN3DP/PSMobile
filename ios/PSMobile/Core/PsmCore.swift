...
final class PsmCore {
    ...
    /// Ob dieser Kern STEP lesen kann - siehe psm_supports_step().
    static var supportsStep: Bool { psm_supports_step() != 0 }
    ...
    /// Session - siehe docs/decisions.md, E-03. Dafuer braucht er
    /// den Zeiger.
    ...
    /// Was der Prozess gerade belegt (phys_footprint) - der Anteil, den
    /// der Speicherwaechter zum verfuegbaren Speicher dazurechnet, weil
    /// der Kern ihn beim Slicen wieder freigeben und neu nutzen kann.
    /// Gegenstueck zu `Debug.getPss()` in SlicerService.kt.
