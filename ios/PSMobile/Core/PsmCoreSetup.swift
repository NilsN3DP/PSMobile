...
extension PsmCore {
    ...
    /// Ein Profil wechseln und ausgewaehlte, bereits bearbeitete Werte
    /// danach wieder eintragen - PrusaSlicers "Transfer"-Weg im
    /// Unsaved-Changes-Dialog.
    func selectPresetKeeping(_ type: PresetType,
                             _ name: String,
                             changes: [DirtyValue]) throws {
        let keys = changes.map { strdup($0.key) }
        let values = changes.map { strdup($0.newValue) }
        defer {
            keys.forEach { free($0) }
            values.forEach { free($0) }
        }
        var cKeys = keys.map { UnsafePointer($0) }
        var cValues = values.map { UnsafePointer($0) }
        try check(psm_preset_select_keeping(raw, cType(type), name,
                                            &cKeys, &cValues, changes.count),
    ...
    // Reihenfolge wie psm_config_type in psmobile_core.h: PERCENT = 5, POINT = 6.
    // Bis zum 16.09.2026 standen points und percent vertauscht - unsichtbar,
    // solange nur bool und enumeration abgefragt wurden.
    enum ConfigType: Int32 { case bool = 0, int, float, string, enumeration, percent, points }
