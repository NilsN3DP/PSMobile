...
struct AppSettingsView: View {
    ...
    @State private var zeigeOpenSource = false
    ...
    var body: some View {
        SchwebenderDialog(kennung: "dialog.appeinstellungen", maximaleBreite: ps.pt(760)) {
            VStack(spacing: 0) {
                ...
                // gleichlautende Stelle in AppSettingsView.kt.
                ScrollView {
                ...
                .accessibilityIdentifier("appeinstellungen.liste")
            }
        }
    ...
    private var ueber: some View {
        VStack(alignment: .leading, spacing: ps.pt(8)) {
            Text(st("About", "Über").uppercased())
                .font(.system(size: ps.font(11), weight: .semibold))
                .foregroundStyle(PrusaColors.textMuted)
                .padding(.top, ps.pt(20))
                .padding(.bottom, ps.pt(6))

            Button { zeigeOpenSource = true } label: {
                HStack(spacing: ps.pt(12)) {
                    VStack(alignment: .leading, spacing: ps.pt(2)) {
                        Text(st("Open source & licenses", "Open Source & Lizenzen"))
                            .font(.system(size: ps.font(14)))
                            .foregroundStyle(PrusaColors.textPrimary)
                        Text(st("Based on PrusaSlicer (AGPL-3.0). Source code on GitHub and the list of third-party components.",
            ...
            .buttonStyle(.plain)
            .accessibilityIdentifier("appeinstellungen.opensource")
            .sheet(isPresented: $zeigeOpenSource) {
                OpenSourceView { zeigeOpenSource = false }
            }
    ...
    private var versionszeile: some View {
        ...
        return Text("Slicer Mobile \(version) (\(build))")
            .font(.system(size: ps.font(11)))
            .foregroundStyle(PrusaColors.textMuted)
...
final class AppSettingsStore: ObservableObject {
    ...
    init() {
        ...
        // machen daraus einen Vergleich zweier Vorgeschichten: am
        // 10.09.2026 standen "Show all beds" und "Automatischer
        // Testbericht" auf den beiden Bildern verschieden, obwohl die
        // Voreinstellung nur einmal existiert (AppSettings.standard).
        //
        // Nur ueber ein Startargument - eine Einstellung in der App waere
        // ein Schalter, mit dem sich versehentlich alles zuruecksetzt.
        if ProcessInfo.processInfo.arguments.contains("-psm-reset-settings") {
            for schalter in AppSettings.shared.toggles {
                UserDefaults.standard.removeObject(forKey: schalter.key)
            }
            UserDefaults.standard.removeObject(forKey: AppSettings.shared.KEY_START_MODE)
            UserDefaults.standard.removeObject(forKey: Self.languageKey)
            // Die Sichtbarkeitsstufe der Einstellungen (Simple/Advanced/
            // Expert) gehoert dazu: sie entscheidet, wie viel eine
            // Einstellungsseite zeigt, und der Bildvergleich verglich
            // sonst zwei Stufen statt zweier Apps.
            UserDefaults.standard.removeObject(forKey: "psm.sichtbarkeit")
        }
