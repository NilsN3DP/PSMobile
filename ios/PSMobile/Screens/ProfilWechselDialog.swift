...
struct ProfilWechselDialog: View {
    ...
    var grund: String = SimpleModeState.shared.text(
        english: "The simple mode works with the values from the profile. These changes are not adjustable there:",
        german: "Der Einfache Modus arbeitet mit den Werten des Profils. Diese Änderungen lassen sich dort nicht einstellen:")
                    ...
                    ForEach(aenderungen) { a in
                        HStack {
                            VStack(alignment: .leading, spacing: 0) {
                                Text(PsUiCatalog.tr(a.bezeichnung))
                                    .font(.system(size: ps.font(12)))
                                    .foregroundStyle(PrusaColors.textPrimary)
                                    .lineLimit(1)
                                Text(bereich(a.art))
