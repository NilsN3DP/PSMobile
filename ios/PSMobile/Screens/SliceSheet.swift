    ...
    @ViewBuilder private var inhalt: some View {
        ...
        case .done(let sekunden, _, _):
            ...
            Text(st("Sliced in \(dauerText(sekunden))",
                    "Gesliced in \(dauerText(sekunden))"))
        ...
        case .failed(let meldung, let laden):
            // Ein gescheiterter Import ist kein gescheiterter Schnitt - auf
            // dem S23 FE stand "Slicing failed" ueber einer kaputten STL.
            titel(laden ? st("Loading failed", "Laden fehlgeschlagen")
                        : st("Slicing failed", "Slicen fehlgeschlagen"))
