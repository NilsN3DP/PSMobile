...
struct WorkflowStartView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: ps.pt(eng ? 20 : 28)) {
                    ...
                    // mit newProject() wieder weg (Zwilling: WorkflowStartView.kt).
                    weiterarbeitenBereich
                    zuletztBereich
                    modusKarten
                    fusszeile
                }
        ...
        .overlay(alignment: .bottomTrailing) { kaffeeLink }
        .confirmationDialog(
    ...
    /// Projekt zu leeren.
    @ViewBuilder private var weiterarbeitenBereich: some View {
        let anzahl = model.objects.count
        if anzahl > 0 {
            VStack(alignment: .leading, spacing: ps.pt(10)) {
                Text(st("Continue where you left off", "Weiterarbeiten, wo du aufgehört hast"))
                    .font(.system(size: ps.font(12), weight: .semibold))
                    .foregroundStyle(PrusaColors.textMuted)
                HStack(spacing: ps.pt(10)) {
                    Text(anzahl == 1 ? st("1 object on the bed", "1 Objekt auf dem Bett")
                                     : st("\(anzahl) objects on the bed", "\(anzahl) Objekte auf dem Bett"))
                        .font(.system(size: ps.font(13)))
                        .foregroundStyle(PrusaColors.textPrimary)
                        .frame(maxWidth: .infinity, alignment: .leading)
                    Button(st("Simple", "Simple"), action: onSimple)
                        .font(.system(size: ps.font(13)))
                        .foregroundStyle(PrusaColors.orange)
                        .accessibilityIdentifier("start.weiter.simple")
                    Button(st("Advanced", "Advanced"), action: onAdvanced)
                        .font(.system(size: ps.font(13)))
                        .foregroundStyle(PrusaColors.orange)
                        .accessibilityIdentifier("start.weiter.advanced")
                }
    ...
    @ViewBuilder private var kaffeeLink: some View {
        let link = Defaults.shared.KAFFEE_LINK
        if !link.isEmpty, let url = URL(string: link) {
            ...
            .buttonStyle(.plain)
            .accessibilityIdentifier("start.kaffee")
        }
    }
    ...
    private func projektOeffnen(inAdvanced: Bool) {
        ...
        // Als Projekt, nicht als Modellimport: load(url:) legte die
        // Objekte neu zentriert aufs aktive Bett, verlor die zweite
        // Instanz und ignorierte die eingebetteten Profile - ein
        // gesichertes Projekt kam so nie so zurueck, wie es gesichert
        // wurde (RandfaelleUITests, 12.09.2026).
