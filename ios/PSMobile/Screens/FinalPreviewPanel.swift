...
struct FinalPreviewOverlay: View {
    ...
    let onEditor: () -> Void
    /// Was oben frei bleiben muss - Kopf und Werkzeugleiste des Simple
    /// Mode. Bis zum 16.09.2026 lag die Karte im kurzen Fenster (Telefon
    /// quer) auf Slice- und Vorschau-Knopf (Zwilling: FinalPreviewPanel.kt).
    var topInset: CGFloat = 0

    @Environment(\.psScale) private var ps
    /// Nach dem Fenster, nicht nach dem Geraet - wie drueben: ein iPhone
    /// quer ist breit genug fuer die Seitenkarte, ein iPad im schmalen
    /// Split View nicht. Bis zum 16.09.2026 entschied das Idiom, und das
    /// iPhone quer bekam ein Blatt von 360 pt auf 393 pt Hoehe.
    private var isPad: Bool {
        ps.windowSize.width >= 600
    }
    ...
    var body: some View {
        ZStack(alignment: isPad ? .trailing : .bottom) {
            Color.clear
            Group {
                // Die feste Hoehe ist zu knapp fuer Kopf, Statistik, beide
                // Schichtregler, Farbmodus und die Rollen-Chips zusammen -
                // ein Scrollcontainer haelt den unteren Regler erreichbar,
                // statt ihn abzuschneiden. Auch die Seitenkarte: im kurzen
                // Fenster passt sie sonst nicht unter die Werkzeugleiste.
            ...
            .frame(maxHeight: isPad
                   ? (ps.windowSize.height - topInset) * 0.85
                   : ps.pt(360))
