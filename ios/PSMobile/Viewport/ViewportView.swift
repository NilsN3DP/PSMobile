...
struct ViewportView: UIViewRepresentable {
    ...
    /// Lage des Ansichtswuerfels: unten links, wo er nichts verdeckt.
    /// `wuerfelHoch` hebt ihn an, wo unten links etwas liegt (Simple Mode:
    /// Leiste mit Import/Zurueck). Gegenstueck: ViewportView.kt.
    var wuerfelHoch: CGFloat = 0
    ...
    func updateUIView(_ v: PSMGLView, context: Context) {
        ...
        if v.wuerfelHoch != wuerfelHoch { v.wuerfelHoch = wuerfelHoch; v.setNeedsLayout() }
...
final class PSMGLView: UIView {
    ...
    /// Der Ansichtswuerfel oben links - dreht sich in zeichne() mit der Kamera.
    private let wuerfel = AnsichtswuerfelView(frame: .zero)
    var wuerfelHoch: CGFloat = 0
    ...
    init(session: OpaquePointer, shaderDir: String) {
        ...
        wuerfel.onAnsicht = { [weak self] ansicht in
            self?.viewport?.setView(ansicht)
            self?.requestRender()
        }
        wuerfel.onOrbit = { [weak self] dx, dy in
            self?.viewport?.orbit(dx: dx, dy: dy)
            self?.requestRender()
        }
        addSubview(wuerfel)
    }
    ...
    func zeichne() {
        ...
        let winkel = vp.cameraAngles()
        wuerfel.setzeWinkel(yaw: winkel.yaw, pitch: winkel.pitch)
    ...
    override func layoutSubviews() {
        ...
        // Ansichtswuerfel unten links, wie Modifier.align(BottomStart)
        // drueben; 84 pt Kantenlaenge.
        let kante: CGFloat = 84
        wuerfel.frame = CGRect(x: bounds.minX + 6, y: bounds.maxY - kante - 6 - wuerfelHoch,
                               width: kante, height: kante)
        bringSubviewToFront(wuerfel)
        EAGLContext.setCurrent(context)
        deleteBuffers()
        createBuffers()
    ...
    private func createBuffers() {
        ...
        // 24 Bit statt 16: mit 16 Bit zerfaellt das Bettmodell bei Nahebene
        // 0,5 mm und Fernebene ab 1000 mm in Keile (auf dem Galaxy S23 FE
        // am 14.09.2026 gesehen; dieselbe Rechnung gilt hier). Alle Apple-
        // GPUs koennen OES_depth24. Gegenstueck: setEGLConfigChooser(...24...)
        // in SceneView.kt.
        glRenderbufferStorage(GLenum(GL_RENDERBUFFER), GLenum(GL_DEPTH_COMPONENT24_OES),
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        /*
         * Wie im Desktop-PrusaSlicer: die Griffe haben Vorrang, aber ein
         * Zug auf dem gewaehlten Objekt selbst verschiebt es - bei jedem
         * Werkzeug. Bis zum 17.09.2026 galt das nur ohne Gizmo; im Simple
         * Mode (immer Verschieben/Drehen/Skalieren) drehte ein Zug auf dem
         * Objekt deshalb nur die Kamera (Nils, iPad). Freie Flaeche bleibt
         * Orbit.
         */
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        let activePoints = (event?.touches(for: self) ?? [])
            .filter { $0.phase != .ended && $0.phase != .cancelled }
            .map { $0.location(in: self) }
        if let anchor = touchGestureAnchor(points: activePoints) {
            // Beim Wechsel von Pinch zu einem Finger darf der alte
            // Mittelpunkt nicht als Startpunkt der Ein-Finger-Geste
            // stehen bleiben. Sonst springt die Kamera beim naechsten
            // Move um die halbe bisherige Fingerbreite.
            lastPoint = anchor.point
            lastSpan = anchor.span
            moved = true
            requestRender()
            return
        }
