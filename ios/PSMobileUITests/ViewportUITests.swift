...
final class ViewportUITests: XCTestCase {
    ...
    func testWuerfelFlaecheAendertDieAnsicht() {
        let flaeche = app.otherElements["viewport"]
        XCTAssertTrue(flaeche.waitForExistence(timeout: 60))
        sleep(3)
        let vorher = bildDaten()
        let oben = app.buttons["wuerfel.oben"]
        XCTAssertTrue(oben.waitForExistence(timeout: 10), "Der Ansichtswuerfel fehlt")
        oben.tap()
        sleep(2)
        XCTAssertNotEqual(vorher, bildDaten(), "Nach dem Tipp auf Oben sieht die Ansicht unveraendert aus")
    }
    ...
    func testZweiFingerZoomen() {
        ...
        sleep(1)
        if vorher == bildDaten() {
            // Die synthetische Spreizgeste des Simulators kommt nicht
            // jedes Mal als zwei Finger an - in vier Gesamtlaeufen am
            // 12.09.2026 zweimal nicht, allein stehend immer. Einmal
            // nachfassen, bevor der Fall etwas ueber die App behauptet.
            flaeche.pinch(withScale: 2.0, velocity: 1.0)
            sleep(1)
        }
    ...
    func testDirekterFingerzugVerschiebtDasObjektOhneMoveGizmo() {
        ...
        // Ohne Auswahl gibt es die Objektleiste gar nicht mehr - also
        // auf ihr Verschwinden pruefen, nicht auf einen ausgegrauten
        // Knopf. isEnabled auf einem Element, das es nicht gibt, wirft
        // (12.09.2026, Lauf 10: "No matches found" statt Ergebnis).
        start.tap()
        XCTAssertTrue(warte(bis: {
            let griff = self.app.buttons["advanced.gizmo.none"]
            return !griff.exists || !griff.isEnabled
    ...
    func testRotateGizmoLaesstDieObjektflaecheDirektZiehen() {
        pruefeGizmoMitDirektzug("advanced.gizmo.rotate")
    }

    func testScaleGizmoLaesstDieObjektflaecheDirektZiehen() {
    ...
    private func pruefeGizmoMitDirektzug(_ kennung: String) {
        let flaeche = app.otherElements["viewport"]
        XCTAssertTrue(flaeche.waitForExistence(timeout: 60))
        sleep(3)
        objektWaehlen()
        ...
        start.tap()
        XCTAssertTrue(warte(bis: {
            let griff = self.app.buttons[kennung]
            return !griff.exists || !griff.isEnabled
        }), "\(kennung): der Zug auf der Objektflaeche liess das Objekt am alten Ort")

        // Am Ziel liegt das Objekt und kann wieder gewaehlt werden.
        ziel.tap()
        XCTAssertTrue(warte(bis: { self.app.buttons[kennung].isEnabled }),
