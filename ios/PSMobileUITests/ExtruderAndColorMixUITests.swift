...
final class ExtruderAndColorMixUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        app = XCUIApplication()
        app.sauberStarten([ "-psm-reset-setup", "-psm-preset-printer", "-psm-test-eight-extruders", "-psm-test-colormix-colors", "-psm-start-simple", ])
    ...
    func testColorMixPreviewsAndSavesWithoutChangingFilament() {
        ...
        XCTAssertTrue(app.descendants(matching: .any)["colormix.recipe.9"].waitForExistence(timeout: 5),
...
final class PrinterCredentialStoreUITests: XCTestCase {
    func testAppStoresCredentialInKeychainNotUserDefaults() {
        ...
        app.sauberStarten(["-psm-reset-setup", "-psm-preset-printer", "-psm-start-simple", "-psm-credential-self-test"])
