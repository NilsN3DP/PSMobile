...
final class KegelUntersuchungTests: XCTestCase {
    ...
    func testKegelVorschau() throws {
        let app = XCUIApplication()
        app.sauberStarten(["-psm-preset-printer", "-psm-start-advanced", "-psm-load-kegel"])
        XCTAssertTrue(app.otherElements["arbeitsbereich"].waitForExistence(timeout: 60))
        ...
        // Im Expertenmodus gibt es nach dem Schneiden kein Blatt mehr,
        // das erst weggetippt werden muesste: "Slice now" wird direkt
        // durch Export und Senden ersetzt (siehe GcodePreviewUITests).
        // Der Test suchte bis zum 11.09.2026 `slice.schliessen` und
        // meldete "Schnitt kam nicht durch" - er suchte einen Knopf, den
        // der Umbau abgeschafft hat.
        let fertig = app.buttons["slice.sichern"]
        XCTAssertTrue(fertig.waitForExistence(timeout: 180), "Schnitt kam nicht durch")
        halte(app, "2a-fertig")
