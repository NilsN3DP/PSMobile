...
object EasyModeState {
    ...
    fun profileSetupAction(kind: EasyProfileKind): String = when (kind) {
        ...
        EasyProfileKind.PRINT_SETTINGS -> SimpleModeState.text(
            "Set up print settings", "Druckeinstellungen einrichten")
    }
                ...
                EasyPrinterModelChoice(
                    ...
                    variants = presets
                        .map { (rawPreset, displayName) ->
                            EasyPrinterChoice(
                                nozzleVariantLabel(displayName.substringAfter(label).removePrefix(" ")),
                                rawPreset,
                            )
                        }
                        .sortedBy { it.label },
    ...
    /**
     * "0.4 nozzle" → "0.4 mm": die Karte schreibt "Düse" davor, und bis zum
     * 16.09.2026 stand dort "Düse  0.4 nozzle" ("Nozzle  0.4 nozzle"). Was
     * keine Duesenangabe ist, bleibt; leer heisst "Standard".
     */
    fun nozzleVariantLabel(variant: String): String {
        val ohne = variant.trim().removeSuffix("nozzle").removeSuffix("Nozzle").trim()
        if (ohne.isBlank()) return "Standard"
        val zahl = Regex("""^(HF\s*)?(\d+(?:[.,]\d+)?)(\s*mm)?$""").matchEntire(ohne)
        return if (zahl != null) (zahl.groupValues[1].trim().let { if (it.isEmpty()) "" else "$it " }) + zahl.groupValues[2] + " mm"
        else ohne
    }
