...
object PrusaLinkRules {
    ...
    fun uploadPath(storage: String, fileName: String): String {
        // Nur ASCII: isLetterOrDigit() liess Umlaute durch, und "Würfel.bgcode"
        // war auf iOS keine gueltige URL (badURL) und ging auf Android roh
        // ueber die Leitung (16.09.2026). Umlaute werden umschrieben, alles
        // andere Nicht-ASCII wird zum Unterstrich.
        val sicher = buildString {
            for (c in fileName) {
                when (c) {
                    'ä' -> append("ae"); 'ö' -> append("oe"); 'ü' -> append("ue")
                    'Ä' -> append("Ae"); 'Ö' -> append("Oe"); 'Ü' -> append("Ue"); 'ß' -> append("ss")
    ...
    fun describeStatus(body: String): String {
        ...
        return buildString {
            append(zustand?.ifBlank { null }?.let(::stateLabel) ?: SimpleModeState.text("connected", "verbunden"))
            if (duese != null) {
    ...
    /**
     * Der Zustand lesbar und in der Sprache der Oberflaeche. Bis zum
     * 16.09.2026 stand das rohe API-Wort auf der Karte ("PRINTING",
     * CORE One L am S23 FE) - in der deutschen Oberflaeche neben "Düse".
     * Unbekannte Woerter bleiben, wie sie kommen.
     */
    fun stateLabel(state: String): String = when (state.trim().uppercase()) {
        "IDLE" -> SimpleModeState.text("Idle", "Bereit")
        "READY" -> SimpleModeState.text("Ready", "Bereit")
        "BUSY" -> SimpleModeState.text("Busy", "Beschäftigt")
        "PRINTING" -> SimpleModeState.text("Printing", "Druckt")
        "PAUSED" -> SimpleModeState.text("Paused", "Pausiert")
        "FINISHED" -> SimpleModeState.text("Finished", "Fertig")
        "STOPPED" -> SimpleModeState.text("Stopped", "Gestoppt")
        "ERROR" -> SimpleModeState.text("Error", "Fehler")
        "ATTENTION" -> SimpleModeState.text("Needs attention", "Braucht Aufmerksamkeit")
        else -> state
    }
