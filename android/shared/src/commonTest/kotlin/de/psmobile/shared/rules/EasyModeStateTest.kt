...
class EasyModeStateTest {
    ...
    @Test
    fun nozzleVariantLabelReadsAsMillimetres() {
        assertEquals("0.4 mm", EasyModeState.nozzleVariantLabel("0.4 nozzle"))
        assertEquals("HF 0.6 mm", EasyModeState.nozzleVariantLabel("HF0.6 nozzle"))
        assertEquals("Standard", EasyModeState.nozzleVariantLabel(""))
        assertEquals("Input Shaper", EasyModeState.nozzleVariantLabel("Input Shaper"))
        assertEquals(
            listOf("0.4 mm", "0.6 mm"),
            EasyModeState.printerModelsWithNozzles(listOf("Prusa CORE One 0.4 nozzle", "Prusa CORE One 0.6 nozzle"))
                .single().variants.map { it.label },
        )
    }
