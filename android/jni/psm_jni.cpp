...
jstring jstr(JNIEnv *env, std::string_view value)
{
    ...
    for (size_t i = 0; i < value.size();) {
        ...
        else return env->NewString(nullptr, 0);
        if (i + need > value.size()) return env->NewString(nullptr, 0);
        for (size_t n = 0; n < need; ++n) {
            const unsigned char c = static_cast<unsigned char>(value[i++]);
            if ((c & 0xC0) != 0x80) return env->NewString(nullptr, 0);
            cp = (cp << 6) | (c & 0x3F);
        }
        if (cp > 0x10FFFF || (cp >= 0xD800 && cp <= 0xDFFF)) return env->NewString(nullptr, 0);
        if (cp <= 0xFFFF) utf16.push_back(static_cast<jchar>(cp));
        else { cp -= 0x10000; utf16.push_back(static_cast<jchar>(0xD800 | (cp >> 10))); utf16.push_back(static_cast<jchar>(0xDC00 | (cp & 0x3FF))); }
    }
...
JNIEXPORT jboolean JNICALL JNI_FN(nativeSupportsStep)(JNIEnv *, jclass)
{
    return psm_supports_step() != 0 ? JNI_TRUE : JNI_FALSE;
}
...
JNIEXPORT jstring JNICALL JNI_FN(nativeBedName)(JNIEnv *env, jclass, jlong h, jint index)
{
    ...
    if (index < 0 || psm_bed_metadata_get(sess(h), static_cast<size_t>(index), &metadata) != PSM_OK)
        return env->NewStringUTF("");
    return jstr(env, std::string_view(metadata.name));
}
...
/* [yaw, pitch] in Radiant - fuer den Ansichtswuerfel. */
JNIEXPORT jfloatArray JNICALL JNI_VP(nativeCameraAngles)(JNIEnv *env, jclass, jlong h)
{
    float werte[2] = {0.f, 0.f};
    psm_viewport_camera_angles(vp(h), &werte[0], &werte[1]);
    jfloatArray out = env->NewFloatArray(2);
    env->SetFloatArrayRegion(out, 0, 2, werte);
    return out;
}
...
/* Die im letzten Bild wirklich aktive Schichthoehen-Darstellung, als
   "min	max" in mm, oder null wenn keine gezeichnet wird. Gegenstueck
   zu `activeLayerVisualization` in PsmViewport.swift. */
JNIEXPORT jstring JNICALL JNI_VP(nativeActiveLayerVisualization)(
    JNIEnv *env, jclass, jlong h)
{
    psm_layer_visualization_info info{};
    if (! psm_viewport_active_layer_visualization(vp(h), &info))
        return nullptr;
    const std::string value =
        std::to_string(info.min_layer_height) + "	" +
        std::to_string(info.max_layer_height);
    return env->NewStringUTF(value.c_str());
}

/* Was der Viewport gerade an Bemalung zeigt - fuer die unsichtbare
   Markierung im Baum der Bedienhilfen, wie `aktualisiereMalmarker` auf
   iOS. "cursor	annotation	mode	shape	radius	facets", oder null wenn
   weder Pinselzeiger noch Markierung im Bild sind. */
JNIEXPORT jstring JNICALL JNI_VP(nativeActivePaintVisualization)(
    JNIEnv *env, jclass, jlong h)
{
    psm_paint_visualization_info info{};
    if (! psm_viewport_active_paint_visualization(vp(h), &info))
        return nullptr;
    const std::string value =
        std::to_string(info.cursor_visible) + "	" +
        std::to_string(info.annotation_visible) + "	" +
        std::to_string(info.mode) + "	" +
        std::to_string(info.shape) + "	" +
        std::to_string(info.radius_mm) + "	" +
        std::to_string(info.annotation_facets);
    return env->NewStringUTF(value.c_str());
}
