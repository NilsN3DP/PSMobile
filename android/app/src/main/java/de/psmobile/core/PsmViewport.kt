 ...
 * direkt aus der Session. Siehe docs/decisions.md, E-03.
 */
class PsmViewport private constructor(private var handle: Long) {
    ...
    companion object {
        ...
        @JvmStatic private external fun nativeCameraAngles(h: Long): FloatArray
        ...
        @JvmStatic private external fun nativeActivePaintVisualization(h: Long): String?
        @JvmStatic private external fun nativeActiveLayerVisualization(h: Long): String?
    ...
    enum class View(val raw: Int) { ISO(0), TOP(1), FRONT(2), BACK(3), LEFT(4), RIGHT(5), BOTTOM(6) }

    fun setView(v: View) = nativeViewPreset(handle, v.raw)

    /** Blickrichtung (yaw, pitch) in Radiant - fuer den Ansichtswuerfel. */
    fun cameraAngles(): Pair<Float, Float> {
        val w = nativeCameraAngles(handle)
        return Pair(w[0], w[1])
    }
     ...
     * (feinste, groebste) Schichthoehe in mm - oder null. Kein
     * CPU-Vorabtest: Shader und Textur muessen im letzten Renderdurchlauf
     * aufgebaut worden sein. Gegenstueck zu `activeLayerVisualization`.
     */
    fun activeLayerVisualization(): Pair<Float, Float>? {
        val f = nativeActiveLayerVisualization(handle)?.split('	') ?: return null
        if (f.size != 2) return null
        val min = f[0].toFloatOrNull() ?: return null
        val max = f[1].toFloatOrNull() ?: return null
        return min to max
    }

    /** Gegenstueck zu `PsmViewport.PaintVisualization` auf iOS. */
    data class PaintVisualization(
        val cursorVisible: Boolean,
        val annotationVisible: Boolean,
        val annotationFacets: Int,
        val mode: PsmCore.PaintMode,
        val shape: PsmCore.PaintShape,
        val radiusMm: Float,
    )

    /**
     * Was im letzten Bild tatsaechlich an Bemalung zu sehen war, oder
     * null wenn weder Pinselzeiger noch Markierung im Bild sind.
     */
    fun activePaintVisualization(): PaintVisualization? {
        val f = nativeActivePaintVisualization(handle)?.split('	') ?: return null
        if (f.size != 6) return null
        val mode = PsmCore.PaintMode.entries.firstOrNull { it.raw == f[2].toIntOrNull() }
            ?: return null
        val shape = PsmCore.PaintShape.entries.firstOrNull { it.raw == f[3].toIntOrNull() }
            ?: return null
        return PaintVisualization(
            cursorVisible = f[0] == "1",
            annotationVisible = f[1] == "1",
            annotationFacets = f[5].toIntOrNull() ?: 0,
            mode = mode, shape = shape,
            radiusMm = f[4].toFloatOrNull() ?: 0f,
        )
    }
