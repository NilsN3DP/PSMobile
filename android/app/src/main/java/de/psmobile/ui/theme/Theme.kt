...
object PrusaColors {
    ...
    // Kleingeschriebene Aliasse: die aus iOS portierten Bildschirme
    // schreiben `PrusaColors.orange` wie in `PrusaColors.swift`.
    @get:JvmName("ios_orange") val orange: Color get() = Orange
    @get:JvmName("ios_orangeDim") val orangeDim: Color get() = OrangeDim
    @get:JvmName("ios_background") val background: Color get() = Background
    @get:JvmName("ios_panel") val panel: Color get() = Panel
    @get:JvmName("ios_panelRaised") val panelRaised: Color get() = PanelRaised
    @get:JvmName("ios_divider") val divider: Color get() = Divider
    @get:JvmName("ios_textPrimary") val textPrimary: Color get() = TextPrimary
    @get:JvmName("ios_textMuted") val textMuted: Color get() = TextMuted
    @get:JvmName("ios_danger") val danger: Color get() = Danger
    @get:JvmName("ios_ok") val ok: Color get() = Ok
    @get:JvmName("ios_sceneGradient") val sceneGradient: Brush get() = SceneGradient
}
...
private fun gestauchteDichte(): Density {
    ...
    // sp laeuft in Compose durch density UND fontScale. Mit der gestauchten
    // density schrumpfte eine Schrift also doppelt: erst um `scale`, dann
    // noch einmal um die gedaempfte Schriftstufe - auf einem Telefon
    // (scale 0,7) blieben 57 % statt der 82 %, die iOS mit `ps.font()`
    // zeichnet. Der Bildvergleich 12/13 vom 13.09.2026 machte das sichtbar:
    // dieselben Bildschirme, auf Android eine Nummer kleiner beschriftet.
    // Deshalb die Stauchung aus dem fontScale wieder herausrechnen.
    return Density(
        density = base.density * scale,
        fontScale = base.fontScale * (1f - (1f - scale) * FONT_FOLLOW) / scale,
    )
}

/*
 * Die portierten Bildschirme schreiben ihre Schriftgroessen einzeln hin
 * (`fontSize = ps.font(8)`), so wie ihr Swift-Original
 * (`.font(.system(size: ps.font(8)))`). Auf iOS legt das beides fest:
 * Groesse und Zeilenhoehe. In Compose nur die Groesse - die Zeilenhoehe
 * kommt weiter aus dem geerbten Textstil, und der ist bodyLarge mit
 * 20 sp. Eine 8-sp-Zeile war damit 20 dp hoch statt rund 9.
 *
 * Sichtbar wurde das an zwei Stellen: die Beschriftung unter "Rueckgaengig"
 * brauchte zwei Zeilen, bekam in einem 48-dp-Knopf aber nur eine und
 * stand als "Objekte importier..." da, waehrend iOS zweizeilig umbrach.
 * Und der Einstellungsdialog fiel rund 10 % hoeher aus als drueben.
 *
 * `TextUnit.Unspecified` laesst Compose die Zeilenhoehe aus den
 * Schriftmetriken ableiten - dieselbe Regel, nach der SwiftUI es tut.
 * Material zeichnet seine eigenen Widgets weiter nach [psTypography];
 * dort gehoert die ausdrueckliche Zeilenhoehe hin.
 */
@Composable
...
fun ScaledOverlay(content: @Composable () -> Unit) {
    CompositionLocalProvider(LocalDensity provides gestauchteDichte()) {
        // Und die Kennungen nach draussen geben, wie im Hauptfenster.
        // `testTagsAsResourceId` steht in MainActivity auf dem Wurzel-Box
        // der Aktivitaet - ein Dialog haengt in einem *eigenen* Fenster
        // und erbt davon nichts. Fuer die Compose-Tests machte das
        // keinen Unterschied (die sehen alle Baeume), fuer alles
        // ausserhalb schon: die Druckermaske erschien in der
        // uiautomator-Ausgabe ohne eine einzige Kennung, waehrend
        // dieselbe Maske auf iOS ihre accessibilityIdentifier
        // selbstverstaendlich mitbringt.
