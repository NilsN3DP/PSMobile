...
android {
    ...
    buildTypes {
        release {
            ...
            val uploadStoreFile = providers.gradleProperty("PSMOBILE_UPLOAD_STORE_FILE").orNull
            val uploadStorePassword = providers.gradleProperty("PSMOBILE_UPLOAD_STORE_PASSWORD").orNull
            val uploadKeyAlias = providers.gradleProperty("PSMOBILE_UPLOAD_KEY_ALIAS").orNull
            val uploadKeyPassword = providers.gradleProperty("PSMOBILE_UPLOAD_KEY_PASSWORD").orNull
            if (!uploadStoreFile.isNullOrBlank() && !uploadStorePassword.isNullOrBlank() &&
                !uploadKeyAlias.isNullOrBlank() && !uploadKeyPassword.isNullOrBlank()) {
...
/*
 * Paritaetswaechter im Bau.
 *
 * Der pre-commit-Hook (tools/hooks) haengt an einer lokalen
 * Git-Einstellung und wandert deshalb NICHT mit dem Repository mit - in
 * einer frisch geklonten Kopie greift er erst nach
 * `git config core.hooksPath tools/hooks`. Genau so eine Luecke hat die
 * beiden Apps schon einmal auseinanderlaufen lassen.
 *
 * Diese Aufgabe haengt am `check`-Ziel und damit an jeder Kopie, die
 * das Repository auscheckt - ohne Einrichtung, ohne Gedaechtnis.
 *
 * Siehe docs/keeping-in-sync.md.
 */
val paritaetPruefen by tasks.registering(Exec::class) {
    group = "verification"
    description = "Vergleicht die Android-Oberflaeche mit der iOS-Referenz"
    workingDir = rootProject.projectDir.parentFile
    val skript = File(workingDir, "tools/paritaet_pruefen.py")
    // Ohne Python (oder ohne iOS-Quellen im Baum) still weitergehen: der
    // Waechter soll den Bau schuetzen, nicht ihn verhindern.
    val python = listOf("python", "python3").firstOrNull { name ->
        runCatching {
            ProcessBuilder(name, "--version").redirectErrorStream(true).start().waitFor() == 0
        }.getOrDefault(false)
    }
    onlyIf {
        val da = skript.isFile && File(workingDir, "ios/PSMobile").isDirectory && python != null
        if (!da) logger.lifecycle("paritaet: uebersprungen (kein Python oder keine iOS-Quellen)")
        da
    }
    commandLine(python ?: "python", skript.absolutePath, "--still")
}

tasks.named("check") { dependsOn(paritaetPruefen) }
